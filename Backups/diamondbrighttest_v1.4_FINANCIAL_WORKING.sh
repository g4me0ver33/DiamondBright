...
read -p "Average Hourly Wage (optional): $" WAGE
...
if [[ "$EMPLOYEES" =~ ^[0-9]+$ ]] && [[ "$WAGE" =~ ^[0-9]+$ ]]; then
    ...
    if [ "$FULL_DAY" -gt 0 ]; then
        DAYS_EQUIV=$((YEARLY_PLAN / FULL_DAY))
        echo "Protection Plan Comparison:"
        echo "Approximately ${DAYS_EQUIV} full-day outage(s)"
        echo "equals one year of protection."
    fi
